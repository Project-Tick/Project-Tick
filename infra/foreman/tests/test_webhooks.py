...
def test_receive_github_webhook_master_push_dispatches(client: TestClient, mock_db):
...
def test_receive_github_webhook_feature_branch_push_is_ignored(client: TestClient, mock_db):
    ...
    assert response_data["message"] == "Webhook received"
    assert response_data["event_id"] == delivery_id
    assert "pipeline_id" not in response_data
...
def test_should_not_store_event_push_to_beta():
    """Test should_store_event returns False for push to beta."""
    from app.routes.webhooks import should_store_event
...
def test_should_not_store_event_push_to_branch():
    """Test should_store_event returns False for push to branch/*."""
    from app.routes.webhooks import should_store_event
...
def test_should_not_store_event_push_to_feature_branch():
    """Test should_store_event returns False for a regular branch push."""
    from app.routes.webhooks import should_store_event
...
@pytest.mark.asyncio
@pytest.mark.no_default_ci_rule
async def test_handle_issue_retry_resolves_workflow_from_log_url_without_retry_trigger():
    from app.routes.webhooks import handle_issue_retry

    event_id = uuid.uuid4()
    pipeline_id = uuid.uuid4()
    issue_body = """The stable build pipeline for `test-app` failed.

Commit SHA: abc123456789
Ref: refs/heads/master
Build log: https://github.com/NeoZip-Infra/foreman/actions/runs/123456789
GitHub repository: Project-Tick/Project-Tick"""
    ...
    rule = WorkflowDispatchRule(
        name="neozip-cmake-push",
        workflow_id="neozip-cmake.yml",
        workflow_name="NEOZIP CMake",
        owner="NeoZip-Infra",
        repo="foreman",
        ref="master",
    ...
    with (
        patch.object(settings, "workflow_dispatch_rules", [rule]),
        patch("app.routes.webhooks.validate_retry_permissions", return_value=True),
        patch("app.routes.webhooks.is_issue_edited", AsyncMock(return_value=False)),
        patch(
            "app.routes.webhooks.get_workflow_run_title",
        ...
        patch(
            "app.routes.webhooks.GitHubActionsService.get_workflow_run_details",
            AsyncMock(
                return_value={
                    "path": ".github/workflows/neozip-cmake.yml@refs/heads/master"
                }
            ),
        ),
        patch("app.routes.webhooks.BuildPipeline", return_value=mock_pipeline_service),
        ...
        create_kwargs = mock_pipeline_service.create_pipeline.await_args.kwargs
        assert create_kwargs["params"]["dispatch_owner"] == "NeoZip-Infra"
        assert create_kwargs["params"]["dispatch_repo"] == "foreman"
        assert create_kwargs["params"]["dispatch_workflow_id"] == "neozip-cmake.yml"
