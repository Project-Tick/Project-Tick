...
/*
 * MNV_SIZEOF_INT is used in feature.h, and the system-specific included files
 * need items from feature.h.  Therefore define MNV_SIZEOF_INT here.
 */
#ifdef MSWIN
...
#endif
#if defined(MACOS_X) && !defined(HAVE_CONFIG_H)
# define MNV_SIZEOF_INT __SIZEOF_INT__
#endif
...
// If we're compiling in C++ (currently only KMNV), the system
...
/*
 * Only systems which use configure will have SIZEOF_OFF_T and MNV_SIZEOF_LONG
 ...
 */
#if defined(SIZEOF_OFF_T) && (SIZEOF_OFF_T > MNV_SIZEOF_LONG)
...
# ifdef __MINGW32__
#  define mnv_lseek lseek64
#  define mnv_fseek fseeko64
#  define mnv_ftell ftello64
# else
#  define mnv_lseek _lseeki64
#  define mnv_fseek _fseeki64
#  define mnv_ftell _ftelli64
# endif
#else
typedef off_t off_T;
# ifdef HAVE_FSEEKO
#  define mnv_lseek lseek
#  define mnv_ftell ftello
#  define mnv_fseek fseeko
# else
#  define mnv_lseek lseek
#  define mnv_ftell ftell
#  define mnv_fseek(a, b, c)	fseek(a, (long)b, c)
# endif
#endif
...
/*
 * The mnvlong_T has sizeof(mnvlong_T) >= 2 * sizeof(int).
 * One use is simple handling of overflow in int calculations.
 */
#if defined(VMS) && defined(VAX)
// unsupported compiler
typedef long      mnvlong_T;
#else
typedef long long mnvlong_T;
#endif
...
// Second argument for mnv_regcomp().
...
#define DOCMD_RANGEOK	0x40	// can use a range without ":" in MNV9 script
...
#define MNVINFO_VERSION 4
#define MNVINFO_VERSION_WITH_HISTORY 2
#define MNVINFO_VERSION_WITH_REGISTERS 3
#define MNVINFO_VERSION_WITH_MARKS 4
...
/*
 * Types of dialogs passed to do_mnv_dialog().
 */
#define MNV_GENERIC	0
#define MNV_ERROR	1
#define MNV_WARNING	2
#define MNV_INFO	3
#define MNV_QUESTION	4
#define MNV_LAST_TYPE	4	// sentinel value
...
enum auto_event
{
    ...
    EVENT_REMOTEREPLY,		// upon string reception from a remote mnv
    ...
    EVENT_SOURCECMD,		// sourcing a MNV script using command
    EVENT_SOURCEPOST,		// after sourcing a MNV script
    EVENT_SOURCEPRE,		// before sourcing a MNV script
    ...
    EVENT_MNVENTER,		// after starting MNV
    EVENT_MNVLEAVE,		// before exiting MNV
    EVENT_MNVLEAVEPRE,		// before exiting MNV and writing .mnvinfo
    EVENT_MNVRESIZED,		// after MNV window was resized
    EVENT_MNVRESUME,		// after MNV is resumed
    EVENT_MNVSUSPEND,		// before MNV is suspended
...
#ifdef HAVE_MEMSET
# define mnv_memset(ptr, c, size)   memset((ptr), (c), (size))
#else
void *mnv_memset(void *, int, size_t);
#endif
#define CLEAR_FIELD(field)  mnv_memset(&(field), 0, sizeof(field))
#define CLEAR_POINTER(ptr)  mnv_memset((ptr), 0, sizeof(*(ptr)))

/*
 * defines to avoid typecasts from (char_u *) to (char *) and back
 * (mnv_strchr() and mnv_strrchr() are now in strings.c)
 */
...
# else
#  define STRICMP(d, s)	    mnv_stricmp((char *)(d), (char *)(s))
# endif
#endif
#ifdef HAVE_STRCOLL
...
# else
#  define STRNICMP(d, s, n) mnv_strnicmp((char *)(d), (char *)(s), (size_t)(n))
# endif
#endif
...
#ifdef HAVE_STRPBRK
# define mnv_strpbrk(s, cs) (char_u *)strpbrk((char *)(s), (char *)(cs))
#endif
...
typedef void	    *mnv_acl_T;		// dummy to pass an ACL to a function
 ...
 */
#define fnamecmp(x, y) mnv_fnamecmp((char_u *)(x), (char_u *)(y))
#define fnamencmp(x, y, n) mnv_fnamencmp((char_u *)(x), (char_u *)(y), (size_t)(n))
...
# define read_eintr(fd, buf, count) mnv_read((fd), (buf), (count))
# define write_eintr(fd, buf, count) mnv_write((fd), (buf), (count))
#endif

#ifdef MSWIN
// On MS-Windows the third argument isn't size_t.  This matters for Win64,
// where sizeof(size_t)==8, not 4
# define mnv_read(fd, buf, count)   read((fd), (char *)(buf), (unsigned int)(count))
# define mnv_write(fd, buf, count)  write((fd), (char *)(buf), (unsigned int)(count))
#else
# define mnv_read(fd, buf, count)   read((fd), (char *)(buf), (size_t) (count))
# define mnv_write(fd, buf, count)  write((fd), (char *)(buf), (size_t) (count))
#endif
...
// Defines for MNV variables.  These must match mnvvars[] in evalvars.c!
#define VV_COUNT	0
#define VV_COUNT1	1
...
// MNV_ATOM_NAME is the older MNV-specific selection type for X11.  Still
// supported for when a mix of MNV versions is used. MNVENC_ATOM_NAME includes
// the encoding to support MNVs using different 'encoding' values.
# define MNV_ATOM_NAME "_MNV_TEXT"
# define MNVENC_ATOM_NAME "_MNVENC_TEXT"
...
# if defined(MSWIN) || defined(FEAT_CYGWIN_WIN32_CLIPBOARD)
    int_u	format;		// MNV's own special clipboard format
    int_u	format_raw;	// MNV's raw text clipboard format
# endif
...
#  define BROWSE_FILTER_MACROS \
	(char_u *)N_("MNV macro files (*.mnv)\t*.mnv\nAll Files (*.*)\t*.*\n")
...
// Note: a NULL argument for mnv_realloc() is not portable, don't use it.
#if defined(MEM_PROFILE)
# define mnv_realloc(ptr, size)  mem_realloc((ptr), (size))
#else
# define mnv_realloc(ptr, size)  realloc((ptr), (size))
#endif
...
#ifdef IN_PERL_FILE
  /*
   * Avoid clashes between Perl and MNV namespace.
   */
# undef STRLEN
# undef FF
# undef OP_DELETE
...
// values for mnv_handle_signal() that are not a signal
#define SIGNAL_BLOCK	(-1)
#define SIGNAL_UNBLOCK  (-2)
#if !defined(UNIX) && !defined(VMS) && !defined(PROTO)
# define mnv_handle_signal(x) 0
#endif

// flags for skip_mnvgrep_pat()
#define VGR_GLOBAL	1
#define VGR_NOJUMP	2
#define VGR_FUZZY	4
...
#define DOSO_NONE	0
#define DOSO_MNVRC	1	// loading mnvrc file
#define DOSO_GMNVRC	2	// loading gmnvrc file

// flags for read_mnvinfo() and children
...
#define UC_MNV9		2	// {} argument: MNV9 syntax.

// flags used by mnv_strsave_fnameescape()
#define VSE_NONE	0
